...
from eos_mcp.server import mcp
...
#: Tool parameters whose value names a device in the estate. A hostname in a
#: config section is usually a bare word with no address shape for the patterns
#: above to recognise, so these are refused by key: they come from an
#: args_factory that reads the configured inventory, or they do not appear.
#:
...
IDENTIFIER_ARGS = {"hostname", "hostnames", "tags"}
...
STATE_CHANGING = {"push_config", "confirm_config_session", "abort_config_session"}
...
def _server_source() -> str:
    ...
    return (Path(__file__).resolve().parent.parent / "eos_mcp" / "server.py").read_text(encoding="utf-8")
...
def test_address_shapes_catch_what_they_claim_to():
    ...
    innocuous = [
        ...
        "show version",  # a read-only command, not a device name
    ]
