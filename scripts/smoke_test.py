...
confirming or aborting a session — is skipped by name in ``smoke_probes.py``,
and the report prints only tool names, statuses and row counts, never payloads,
which would otherwise carry configuration into logs.

Usage:
    # In-process: imports the server, uses its configured device inventory.
    EOS_MCP_CONFIG=... uv run python scripts/smoke_test.py
...
def main() -> None:
    ...
    # Serial by default: these are production switches, several tools fan
    # out across the whole inventory already, and eAPI sessions are not free —
    # a burst of them is a worse neighbour than a slow smoke test.
